...
fn reject_plaintext_in_encrypted_room(
    ...
    if room_is_encrypted(room_id, ephemeral_rooms, store)
        && !clawchat_core::crypto::is_ciphertext(content)
    {
        return Some(Frame::error(
            req_id,
            ErrorPayload::new(
                ErrorCode::PlaintextInEncryptedRoom,
                "This room is end-to-end encrypted; set CLAWCHAT_ROOM_KEY so the client encrypts before sending",
            ),
        ));
    }
    None
...
async fn handle_thinking(
    ...
    // Thinking shares the same rate-limit bucket as send_message — an agent can't
    // dodge limits by routing chat through `thinking`.
    if !no_auth && !agent_api_key.is_empty() {
        let tier = store

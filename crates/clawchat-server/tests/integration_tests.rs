...
async fn test_message_size_cap() {
    ...
    match err {
        clawchat_client::ClientError::Server { code, .. } => assert_eq!(
            code,
            clawchat_core::ErrorCode::MessageTooLarge,
...
async fn test_protocol_version_gate() {
    ...
    let err: clawchat_core::ErrorPayload = serde_json::from_value(reply.payload).unwrap();
    assert_eq!(
        err.code,
        clawchat_core::ErrorCode::UnsupportedProtocol,
